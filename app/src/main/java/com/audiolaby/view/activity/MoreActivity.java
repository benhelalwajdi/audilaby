...
public class MoreActivity extends BasePlayerActivity {
    ...
    void afterViewsInjection() {
        ...
        if (type.equals(TypeField.tag.name()))
            title_.setText("#"+this.title);
        else
            title_.setText(this.title != null ? this.title : StringUtils.EMPTY);
        this.user = this.libraryDAO.getUser();
        this.adapter = new AudioPostAdapter(MoreActivity.this, this.layout, false, isSorted, new ArrayList());
    ...
    void loadPosts(final int page) {
        if (!isRefresh)
            showProgress();
        isRefresh = false;
        Pagination pagination = new Pagination(page, POST_MORE_SIZE, sort, id);
